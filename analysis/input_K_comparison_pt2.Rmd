...
# Introduction
In this analysis, we include visualizations for one simulation from the DSC. In this example, all of the EB methods use the generalized binary prior.
...
```{r}
est <- readRDS(paste0('data/symnmf_cov_comp_dsc_2k_ex/', codesymnmf_files[5]))$est_L
plot_heatmap(est, title = 'codesymnmf', 
               colors_range = c('blue','gray96','red'),
               brks = seq(-max(abs(est)), max(abs(est)), length.out = 50))
```
