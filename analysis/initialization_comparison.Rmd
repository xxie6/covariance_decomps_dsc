...
dscout <- dscout %>% filter(initialization.K_factor == 1, (is.na(score.threshold) == TRUE | score.threshold == 0.9))
dim(dscout)
```
...
```{r}
dot_plot(bal_nonoverlap_prop_results, 'Proportion Recovered for Balanced Nonoverlapping', '', '')
```

We generally see the same trends with this metric.
...
We generally see the same trends with the proportion recovered metric.
...
With respect to the proportion of factors recovered, the CoDesymNMF initialization generally performs the best. However, with respect to this metric, the point-Laplace initialization generally performs the worst.
...
With respect to the crossproduct similarity, the performance is mixed.
...
With respect to the proportion recovered metric, the point-Laplace plus splitting initialization performs the best and the CoDesymNMF initialization performs the worst. The point-Laplace plus splitting initialization does not seem to return extra factors; the CoDesymNMF and EBMFcov greedy with refitting initializations were given the correct number of components. 
